// Game.cpp — Updated with Exception Handling
...
bool Game::isValideSelectio(int fromRow, int fromCol)
{
	...
	{
		cout << "Invalid input. Square is outside the board." << endl;
		return false;
	}
	Piece* p = board.getPiece(fromRow, fromCol);
	if (p == nullptr)
	{
		cout << "No piece at that square. Try again." << endl;
		return false;
	}
...
void Game::start()
{
	...
	while (!gameOver)
	{
		...
		try
		{
			...
			if (fromInput[0] == '\0' || fromInput[1] == '\0' ||
				toInput[0]   == '\0' || toInput[1]   == '\0')
				throw InvalidMoveException("Invalid input format. Use format: e2 e4");

			if (fromInput[0] < 'a' || fromInput[0] > 'h' ||
				toInput[0]   < 'a' || toInput[0]   > 'h')
				throw InvalidMoveException("Invalid column. Columns must be between a and h.");

			if (fromInput[1] < '1' || fromInput[1] > '8' ||
				toInput[1]   < '1' || toInput[1]   > '8')
				throw InvalidMoveException("Invalid row. Rows must be between 1 and 8.");
			...
			if (!isValideSelectio(fromRow, fromCol))
				continue;
			...
			if (!p->isValidMove(toRow + 1, toCol + 1, board.getGrid()))
				throw InvalidMoveException("Invalid move for that piece.");

			// ── Validate check conditions ─────────────────────────
			if (board.isKingInCheck(player[currentPlayer].getColor()))
			{
				if (board.doesMoveResolveCheck(fromRow, fromCol, toRow, toCol,
					player[currentPlayer].getColor()))
					throw InvalidMoveException(
						"You are in check! This move does not resolve the check.");
			}
			else
			{
				if (board.isInCheckAfterMove(fromRow, fromCol, toRow, toCol,
					player[currentPlayer].getColor()))
					throw InvalidMoveException("This move leaves your King in check!");
			}
			...
			if (destination != nullptr)
				cout << player[currentPlayer].getName()
				     << " captures opponent's piece at " << toInput << "!" << endl;
			...
			if (board.isDraw())
			{
				...
				{ board = Board(); currentPlayer = 0; gameOver = false; }
				else
				{ cout << "Thanks for playing. Goodbye!" << endl; gameOver = true; }
				continue;
			}
		...
		{
			// Catches all invalid move errors thrown above
			cout << ">> " << e.getMessage() << " Try again." << endl;
			continue;
		}
		catch (out_of_range& e)
		{
			// Catches any board coordinate out of bounds
			cout << ">> Board error: " << e.what() << " Try again." << endl;
			continue;
		}
	}
}
